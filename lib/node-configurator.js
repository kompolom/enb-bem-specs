...
exports.configure = function (config, options) {
    ...
    const templateEngineOpts = Object.assign({}, templateEngine.templateOptions, {
        target: '?.template.js',
        filesTarget: '?.template.files'
    }),

    bemtreeOpts = Object.assign({}, templateEngineOpts, {
    ...
    config.nodes(pattern, function (nodeConfig) {
        ...
        if (useBEMTREE) {
            nodeConfig.addTechs([
                [depsByTechToBemdecl, {
                    target: '?.js.bemtree.bemdecl.js',
                    sourceTech: 'js',
                    destTech: 'bemtree'
                }],
                [depsByTechToBemdecl, {
                    target: '?.spec-js.bemtree.bemdecl.js',
                    sourceTech: 'spec.js',
                    destTech: 'bemtree'
                }],
                [mergeBemdecl, {
                    target: '?.bemtree.bemdecl.js',
                    sources: ['?.js.bemtree.bemdecl.js', '?.spec-js.bemtree.bemdecl.js']
                }],
                [bemtreeTech, bemtreeOpts]
            ]);
            templateBemdeclNames.push('?.bemtree.bemdecl.js');
        }
        ...
        nodeConfig.addTechs([
            ...
            [mergeBemdecl, {
                ...
                sources: templateBemdeclNames
            }],
        ]);

        // client template deps
        nodeConfig.addTechs([
            [depsTech, {
        ...
        let browserJSsources = [];
        if (options.langs) { browserJSsources.push('?.i18n.js'); }
        if (useBEMTREE) { browserJSsources.push('?.browser.bemtree.js'); }
        browserJSsources.push('?.template.js'); // Нужно, чтобы файлы добавились именно в таком порядке

        // HTML
        nodeConfig.addTechs([
            [mergeFiles, {
                target: '?.browser.template.js',
                sources: browserJSsources
            }],
        ...
        // CSS
        nodeConfig.addTech([cssEngine.tech, Object.assign({}, cssEngine.options, {
            target: '?.css',
            filesTarget: '?.files'
        })]);
